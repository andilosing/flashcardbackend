...
const manageLearningSession = async (user_id) => {
  try {
    const lastSession =
      await learningSessionsModel.getLastLearningSessionForUser(user_id);
    const currentTime = new Date();

    if (
      !lastSession ||
      currentTime - new Date(lastSession.end_learning_at) > SESSION_THRESHOLD
    ) {
...
const getAllLearningSessions = async (user_id) => {
  try {
    const sessions = await learningSessionsModel.getAllLearningSessionsForUser(
      user_id
    );
...
const calculateAverageLearningTime = async (user_id) => {
  try {
    const firstLearningDateResult =
      await learningSessionsModel.getFirstLearningDateForUser(user_id);
    ...
    const totalLearningTimeResult =
      await learningSessionsModel.getTotalLearningTimeSinceDate(
        user_id,
        firstLearningDateResult.first_learning_date
      );
    const totalLearningTimeMinutes =
      totalLearningTimeResult.total_learning_time_minutes || 0;

    const today = new Date();
    //weil datum falsch aus db tabelle geholt wird um 1 tag
    //today.setDate(today.getDate() - 1);

    const firstLearningDate = new Date(
      firstLearningDateResult.first_learning_date
    );
    const daysSinceFirstLearningDay = Math.ceil(
      (today - firstLearningDate) / (1000 * 60 * 60 * 24)
    );

    const averageLearningTimePerDayMin =
      daysSinceFirstLearningDay > 0
        ? totalLearningTimeMinutes / daysSinceFirstLearningDay
        : 0;
